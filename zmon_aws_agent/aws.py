...
from zmon_aws_agent.common import call_and_retry, trace_span
...
def get_instance_events(aws_client, instance):
    with trace_span('get_instance_events'):
        try:
...
def get_running_apps(region, existing_entities=None):
    with trace_span('get_running_apps') as span:
        ...
        for r in rs:
            ...
            for i in instances:
                ...
                if (now.minute % 7) and i['InstanceId'] in existing_instances:
                    ...
                    user_data = None
                    with trace_span('describe_instance_attribute', parent=span):
                        try:
            ...
            if list(images):
                with trace_span('describe_images', parent=span):
                    try:
                        ...
                        for i in result:
                            ...
                            for img in imgs:
                                if img['ImageId'] == i['image']['id']:
                                    ...
                                    date = img.get('CreationDate', '1970-01-01T00:00:00.000+00:00')
...
def get_running_elbs_classic(region, acc):
    with trace_span('describe_load_balancers_classic') as span:
        ...
        for e in elbs:
            ...
            with trace_span('describe_instance_health', parent=span):
                try:
...
def get_running_elbs_application(region, acc):
    with trace_span('describe_load_balancers_application') as span:
        ...
        for e in elbs:
            ...
            name = e['LoadBalancerName']
            with trace_span('describe_target_groups', parent=span):
            ...
            with trace_span('describe_listeners', parent=span):
            ...
            for tg in target_groups:
                with trace_span('describe_target_health', parent=span):
                    try:
...
def get_auto_scaling_groups(region, acc):
    ...
    with trace_span('describe_auto_scaling_groups') as span:
        ...
        asgs = call_and_retry(
            lambda: paginator.paginate(
                PaginationConfig={'MaxItems': MAX_PAGE}).build_full_result()['AutoScalingGroups'])
        ...
        for g in asgs:
            ...
            if instance_ids:
                ...
                with trace_span('describe_instances', parent=span):
                    try:
                        reservations = call_and_retry(
                            lambda: ec2_paginator.paginate(
                                InstanceIds=instance_ids).build_full_result()['Reservations'])
...
def get_elasticache_nodes(region, acc):
    with trace_span('get_elasticache_nodes'):
...
def get_dynamodb_tables(region, acc):
    ...
    with trace_span('get_dynamodb_tables') as span:
        ...
        try:
            ...
            for tn in ts:
                with trace_span('describe_table', parent=span):
...
def get_rds_instances(region, acc, existing_entities):
    with trace_span('get_rds_instances'):
        entities = []
...
def get_certificates(region, acc):
    with trace_span('get_certificates') as span:
        ...
        try:
            ...
            for cert in acm_certs:
                with trace_span('describe_certificate', parent=span):
...
def get_limits(region, acc, apps, elbs):
    ...
    with trace_span('get_limits') as span:
        with trace_span('ec2-describe_account_attributes', parent=span):
            try:
        ...
        with trace_span('rds-describe_account_attributes', parent=span):
            try:
        ...
        with trace_span('asg-describe_account_attributes', parent=span):
            try:
        ...
        with trace_span('iam-get_account_summary', parent=span):
            try:
...
def get_sqs_queues(region, acc, all_entities=None):
    with trace_span('get_sqs_queues') as span:
        if all_entities is None:
            all_entities = []
        sqs_queues = []

        try:
            with trace_span('list-sqs-queues'):
                ...
                for queue_url in list_queues_response.get('QueueUrls', []):
                    try:
                        ...
                        if existing_entity and (datetime.now().minute % 15):
                            ...
                            with trace_span('sqs-get_queue_attributes', parent=span):
                                attributes_response = call_and_retry(sqs_client.get_queue_attributes,
                                                                     QueueUrl=queue_url,
                                                                     AttributeNames=['All'])
                                ...
                                if len(arn_tokens) == 6:
                                    ...
                                    logger.error('Illegal SQS queue ARN: "%s" while processing url %s',
                                                 queue_arn, queue_url)
                                    continue
                                ...
                                sqs_entity = {
                                    ...
                                    'receive_messages_wait_time_seconds': int(
                                ...
                                dl_sources_response = call_and_retry(
                                    sqs_client.list_dead_letter_source_queues, QueueUrl=queue_url)
